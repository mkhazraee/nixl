...
- `basic_two_peers.py`: Two peers demonstrating registration, metadata exchange, parallel READs (prepped), parallel WRITEs (initialized), and reposting READs with notifications.
- `nixl_api_example.py`: Broader API tour including repeated postings and combined/prepped flows.
...
Start target:
```bash
python3 examples/python/basic_two_peers.py --mode target --ip 127.0.0.1 --port 5555 --backend UCX
```
Then start initiator:
```bash
python3 examples/python/basic_two_peers.py --mode initiator --ip 127.0.0.1 --port 5555 --backend UCX
```
Options:
