...
"""
Replica of test_nixl_bindings.py::test_agent using nixlServiceAgent in DIRECT mode.

Only changes from the original:
  - nixlAgentConfig  → nixlServiceAgentConfig  (+ defaultMode = DIRECT)
  - nixlAgent        → nixlServiceAgent
Lines not supported by nixlServiceAgent are commented out with #.
"""

import os

import nixl._bindings as nixl
import nixl._service_bindings as nixl_svc
...
def test_service_agent_direct():
    os.environ["NIXL_TELEMETRY_ENABLE"] = "y"
    name1 = "Agent1"
    name2 = "Agent2"

    devices = nixl_svc.nixlServiceAgentConfig()
    devices.useProgThread = False
    devices.defaultMode = nixl_svc.DIRECT

    agent1 = nixl_svc.nixlServiceAgent(name1, devices)
    agent2 = nixl_svc.nixlServiceAgent(name2, devices)
...
def test_marshal_query():
    """MarshalRequirements from marshalQuery() has the expected staging entry."""
    reqs = nixl_svc.marshalQuery()
    assert isinstance(reqs, nixl_svc.MarshalRequirements)
    # staging_backend always adds one entry with mode=STAGE_BOTH, subKey=None, slotSize=0
    assert len(reqs.entries) >= 1
    staging = next(e for e in reqs.entries if e.mode == nixl_svc.STAGE_BOTH)
    assert staging.subKey is None
    assert staging.slotSize == 0
